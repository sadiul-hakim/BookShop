...
public class AddToCartServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            List<Cart> oldCartList = (List) session.getAttribute(KEY);
            Cart newCart = new Cart( bookId, 1, book.getName(), book.getPrice(), book.getPhoto());

            if (oldCartList == null) {
                List<Cart> newCartList = new ArrayList<>();
                newCartList.add(newCart);
                session.setAttribute(KEY, newCartList);
            } else {
                oldCartList.add(newCart);
                
            }
            
            response.sendRedirect("showCartResult.jspx");
        }
    }
